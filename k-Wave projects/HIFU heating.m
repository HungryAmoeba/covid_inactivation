...
medium.sound_speed = makecircle(Nx,Ny,1500,343,108,108,10);  % [m/s]
  % [m/s]

medium.density     = makecircle(Nx,Ny,997,1.275,108,108,10);  % [kg/m^3]
...
ppw = 1000 / (freq * dx); % points per wavelength %change 1000 to med sound speed that produces smallest time step
cfl = 0.3;                              % cfl number
...
t_end = sqrt( kgrid.x_size.^2 + kgrid.y_size.^2 ) / 1000; %change 1000 to med sound speed that produces smallest time step
Nt = round(t_end / dt);
...
input_args = { 'PlotLayout', true, 'PMLInside', false, 'PlotPML', false, 'DisplayMask', ...
