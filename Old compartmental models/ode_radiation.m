...
dS = -beta.*(I+eta.*A).*S./N - beta.*((1-eps).*I_R+(1-eps).*eta.*A_R).*S./N;
dE = beta.*(I+eta.*A).*S./N + beta.*((1-eps)*I_R + (1-eps)*eta*A_R).*S/N- sigma.*E;
...
dS_R = -beta.*(1-eps)*(I+eta*A).*S_R./N-beta.*(1-eps)*((1-eps).*I_R + (1-eps)*eta.*A_R)*S_R/N;
dE_R = beta.*(1-eps)*(I+eta*A).*S_R./N + beta.*(1-eps)*((1-eps)*I_R+(1-eps)*eta*A_R).*S_R./N - sigma.*E_R;
